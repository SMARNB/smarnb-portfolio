...
def zy_build():
    ...
    try:
        ...
        SXY = 2.5  # footprint scale: stored ORIGINAL 40x40 -> 100x100, whole site follows (idempotent)
        ...
        PX0 = x0 + 7.0                 # pool hugs the north-west quadrant, wall-relative
        PX1 = PX0 + 16.0
        PY0, PY1 = 20.0, 28.0
        ...
        gx0, gx1 = g["x0"] * SXY, g["x1"] * SXY   # site scales with the establishment
        gy0, gy1 = g["y0"] * SXY, g["y1"] * SXY
        ...
        trow = y0 + 3.0                # treadmill row hugs the front wall
        for i, tx in enumerate((x0 + 8, x0 + 14, x0 + 20, x0 + 26)):
            n = i + 1
            rbox("ZoneA_Treadmill_%d_Base" % n, tx - 0.45, tx + 0.45, trow - 1.1, trow + 1.1, ft, ft + 0.22, m_metal, zoneA)
            rbox("ZoneA_Treadmill_%d_Col" % n, tx - 0.06, tx + 0.06, trow - 1.0, trow - 0.88, ft + 0.22, ft + 1.25, m_metal, zoneA)
            rbox("ZoneA_Treadmill_%d_Console" % n, tx - 0.33, tx + 0.33, trow - 1.08, trow - 0.8, ft + 1.25, ft + 1.39, m_red, zoneA)
        rbox("ZoneA_Rack_Shelf_1", x0 + 0.4, x0 + 0.9, y0 + 5.0, y0 + 7.4, ft + 0.35, ft + 0.42, m_metal, zoneA)
        rbox("ZoneA_Rack_Shelf_2", x0 + 0.4, x0 + 0.9, y0 + 5.0, y0 + 7.4, ft + 0.75, ft + 0.82, m_metal, zoneA)
        rbox("ZoneA_Rack_Side_1", x0 + 0.4, x0 + 0.9, y0 + 5.0, y0 + 5.12, ft, ft + 0.9, m_metal, zoneA)
        rbox("ZoneA_Rack_Side_2", x0 + 0.4, x0 + 0.9, y0 + 7.28, y0 + 7.4, ft, ft + 0.9, m_metal, zoneA)
        for i, dy in enumerate((y0 + 5.4, y0 + 6.0, y0 + 6.6, y0 + 7.2)):
            dz = ft + 0.49 if i % 2 == 0 else ft + 0.89
            cyl("ZoneA_Dumbbell_%d" % (i + 1), x0 + 0.65, dy, dz, 0.07, 0.35, m_teal, zoneA, rot=(0, 1.5708, 0))
        rbox("ZoneA_Bench_1", -31.0, -29.5, -16.0, -15.5, ft, ft + 0.48, m_wood, zoneA)
        rbox("ZoneA_Bench_2", -27.0, -25.5, -16.0, -15.5, ft, ft + 0.48, m_wood, zoneA)
        rbox("ZoneA_Mat_1", -8.0, -7.0, -12.0, -10.0, ft + 0.04, ft + 0.075, m_teal, zoneA)
        rbox("ZoneA_Mat_2", -6.4, -5.4, -12.0, -10.0, ft + 0.04, ft + 0.075, m_amber, zoneA)
        rbox("ZoneA_PullupRig_Post_1", 0.45, 0.55, y1 - 3.55, y1 - 3.45, ft, ft + 2.2, m_metal, zoneA)
        rbox("ZoneA_PullupRig_Post_2", 2.45, 2.55, y1 - 3.55, y1 - 3.45, ft, ft + 2.2, m_metal, zoneA)
        cyl("ZoneA_PullupRig_Bar", 1.5, y1 - 3.5, ft + 2.15, 0.03, 2.0, m_metal, zoneA, rot=(0, 1.5708, 0))

        # ---------- Zone B: cafeteria ----------
        rbox("ZoneB_Cafe_Floor", xdiv + 0.15, x1 - T, y0 + T, y1 - T, ft, ft + 0.04, m_cafe, zoneB)
        accents = (m_red, m_amber, m_teal)
        span = x1 - xdiv
        for i, sx in enumerate((xdiv + span * 0.25, xdiv + span * 0.5, xdiv + span * 0.75)):
            n = i + 1
            acc = accents[i]
            rbox("ZoneB_Stall_%d_Counter" % n, sx - 1.7, sx + 1.7, y1 - 3.1, y1 - 2.0, ft, ft + 1.05, acc, zoneB)
            rbox("ZoneB_Stall_%d_CounterTop" % n, sx - 1.8, sx + 1.8, y1 - 3.2, y1 - 1.9, ft + 1.05, ft + 1.13, m_wood, zoneB)
            rbox("ZoneB_Stall_%d_Panel" % n, sx - 1.7, sx + 1.7, y1 - 0.6, y1 - 0.45, ft, ft + 2.4, acc, zoneB)
            rbox("ZoneB_Stall_%d_Sign" % n, sx - 1.3, sx + 1.3, y1 - 0.7, y1 - 0.6, ft + 1.7, ft + 2.2, m_wood, zoneB)
        for i, (tx, ty) in enumerate(((18, -10), (28, -2), (38, -16), (20, 8))):
            n = i + 1
        ...
        for i, lx in enumerate((25, 35)):
            n = i + 1
            rbox("ZoneB_LongTable_%d_Top" % n, lx - 1.4, lx + 1.4, 28.05, 28.95, ft + 0.72, ft + 0.8, m_wood, zoneB)
            rbox("ZoneB_LongTable_%d_Leg_1" % n, lx - 1.3, lx - 1.15, 28.15, 28.85, ft, ft + 0.72, m_metal, zoneB)
            rbox("ZoneB_LongTable_%d_Leg_2" % n, lx + 1.15, lx + 1.3, 28.15, 28.85, ft, ft + 0.72, m_metal, zoneB)
            rbox("ZoneB_LongBench_%d_1" % n, lx - 1.4, lx + 1.4, 27.35, 27.7, ft, ft + 0.45, m_wood, zoneB)
            rbox("ZoneB_LongBench_%d_2" % n, lx - 1.4, lx + 1.4, 29.3, 29.65, ft, ft + 0.45, m_wood, zoneB)
        ...
        for k in range(8):
            ...
            for rname in rooms:
                ...
                if ob is not None:
                    lx = ob.location.x
                    side = 0 if abs(lx) < 5.0 else (1 if lx > 0 else -1)
                    nx = (10.0 if side != 0 else 20.0) * SXY   # classify by grid slot, not size (idempotent)
                    ob.dimensions = (nx, 20.0 * SXY, H)
                    ob.location.x = side * 15.0 * SXY
                    ob.location.y = (10.0 * SXY) if ob.location.y > 0 else (-10.0 * SXY)
                    ob.location.z = zc
                    restacked += 1
        lobby = bpy.data.objects.get("Grand_Lobby_4_Story")
        if lobby is not None:
            lobby.dimensions = (20.0 * SXY, 20.0 * SXY, 4 * H)
            lobby.location.x = 0.0
            lobby.location.y = -10.0 * SXY
        ...
        for i, (w, sy) in enumerate(stair_spec):
            ...
            if ob is not None:
                d = ob.dimensions
                ob.dimensions = (w * SXY, 1.0 * SXY, d.z)
                ob.location.y = sy * SXY
                restacked += 1
        # site: fences raised to guard-room height (all sides), grassland + its
        # stairs + guard house scaled in x/y. All values are ORIGINAL pre-split
        # analysis numbers * SXY (absolute -> idempotent). Local dims of value 0
        # (flat walls) are left untouched via the scale-ratio method.
        site_spec = (
            ("Building_frontsidefencewall", -4.97, -46.56, 50.06, 0.0, 5.39, H),
            ("Building_rightsidefencewall", 30.05, -3.69, 66.63, 0.0, 5.29, H),
            ("Building_leftsidefencewall", -30.0, -8.41, 76.73, 0.0, 5.29, H),
            ("Grassland_frontsidewall", -7.19, 30.02, 45.97, 0.0, 3.81, H),
            ("Grassland_backsidewall", 0.1, 46.11, 60.3, 0.0, 4.02, H),
            ("Grassland_rightsidewall", 30.24, 37.77, 16.32, 0.0, 3.96, H),
            ("Grassland_leftsidewall", -30.11, 38.18, 16.32, 0.0, 3.96, H),
            ("Grassland", -0.02, 38.06, 60.49, 16.06, None, None),
            ("Grassland_Stairs_1", 23.6, 28.86, 9.76, 1.0, None, None),
            ("Grassland_Stairs_2", 23.6, 29.36, 10.57, 1.0, None, None),
            ("Grassland_Stairs_3", 23.6, 30.11, 11.88, 1.0, None, None),
            ("Grassland_Stairs_4", 23.6, 30.52, 12.44, 1.0, None, None),
            ("Enternace_Area", 25.12, -41.85, 10.0, 9.46, 5.3, H),
        )
        for name, lx, ly, dx, dy, zbot, hgt in site_spec:
            ob = bpy.data.objects.get(name)
            if ob is None:
                continue
            d = ob.dimensions
            tgt = (dx * SXY, dy * SXY, hgt if hgt is not None else d.z)
            cur = (d.x, d.y, d.z)
            s = ob.scale
            ob.scale = tuple(s[i] * (tgt[i] / cur[i]) if cur[i] > 1e-6 else s[i] for i in range(3))
            ob.location.x = lx * SXY
            ob.location.y = ly * SXY
            if zbot is not None and hgt is not None:
                ob.location.z = zbot + hgt / 2
            restacked += 1
